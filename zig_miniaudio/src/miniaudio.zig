...
fn dataCallback(device: *ma.ma_device, output: *anyopaque, input: [*]const anyopaque, frameCount: ma.ma_uint64) void {
    // var decoder = device.pUserData;
    if (device.pUserData) |decoder| {
        ma.ma_decoder_read_pcm_frames(decoder, output, frameCount, null);
    }
    _ = input;
}

pub fn play(filepath: [*c]const u8) !void {
    var decoder = ma.ma_decoder{};
    const res = ma.ma_decoder_init_file(filepath, null, &decoder);
...
//         const group = try engine.allocator.create(ma.ma_sound_group);
//
//         const res = ma.ma_sound_group_init(engine.engine, null, group);
//         if (res == ma.MA_SUCCESS)
...
//         const group = try engine.allocator.create(ma.ma_sound_group);
//
//         const res = ma.ma_sound_group_init(engine.engine, parent.group, group);
//         if (res == ma.MA_SUCCESS)
//             return SoundGroup{ .allocator = engine.allocator, .group = group };
//
//         engine.allocator.destroy(group);
...
//     }
//
//     pub fn setStartDelay(self: *@This(), delay: c_ulonglong) void {
//         _ = ma.ma_sound_group_set_start_delay(self.group, delay);
//     }
//
...
//             if (self.stream) return ma.MA_DATA_SOURCE_FLAG_DECODE | ma.MA_DATA_SOURCE_FLAG_STREAM | ma.MA_DATA_SOURCE_FLAG_ASYNC;
//             return ma.MA_DATA_SOURCE_FLAG_DECODE;
//         }
//     };
//
...
//         const sound = try engine.allocator.create(ma.ma_sound);
//
//         const group = if (options.group) |g| g.group else null;
//         const res = ma.ma_sound_init_from_file(engine.engine, path, options.getFlags(), null, group, sound);
//         if (res == ma.MA_SUCCESS) {
...
//         comptime {
//             const first_field = std.meta.fields(std.meta.Child(@TypeOf(data_source)))[0];
//             std.debug.assert(first_field.field_type == ma.ma_data_source_callbacks);
//         }
//
...
//         const waveform = engine.allocator.create(ma.ma_waveform) catch unreachable;
...
//         const res = ma.ma_waveform_init(&config, waveform);
//         if (res != 0) {
...
//         _ = ma.ma_sound_set_fade_point_in_milliseconds(self.sound, fade_point_index, volume_beg, volume_end, time_in_ms_beg, time_in_ms_end);
//     }
//
...
//
//     pub fn setStartDelay(self: *@This(), delay: c_ulonglong) void {
//         _ = ma.ma_sound_set_start_delay(self.sound, delay);
//     }
//
//     pub fn setStopDelay(self: *@This(), delay_in_ms: c_ulonglong) !void {
...
//     }
//
//     pub fn getTimeInFrames(self: *@This(), _: c_ulonglong) c_ulonglong {
...
//         return @floatFromInt(frames / format.sample_rate);
//     }
// };
//
