...
test "self.writer:" {
    const url = URL{ .port = 80, .path = "127.0.0.1", .schema = "http://" };
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    var res = Response{ .arena = arena.allocator() };
    try std.json.stringify(url, .{}, Writer.init(&res));

    std.debug.print("res.buffer:{s}\n", .{res.buffer.data[0..res.buffer.pos]});
    defer arena.deinit();
}
