...
pub const APK = struct {
    ...
    fn doInstallApk(apk: *@This()) std.mem.Allocator.Error!*Step.InstallFile {
        ...
        std.debug.print("resources_apk:{any}\n", .{resources_apk});
        ...
        if (apk.java_files.items.len > 0) {
            std.debug.print("apk.tools.java_tools.javac:{s}\n", .{apk.tools.java_tools.javac});
        ...
        // See: https://musteresel.github.io/posts/2019/07/build-android-app-bundle-on-command-line.html
        {
            ...
            // jar.addPrefixedFileArg("", resources_apk);
            ...
            std.debug.print("extracted_apk_dir:{s}\n", .{extracted_apk_dir.generated.sub_path});
            // std.debug.print("extracted_apk_dir:{s}\n", .{extracted_apk_dir.generated.file.path.?});
