...
pub const CortexM3Interrupt = enum(u4) {
    ...
    pub fn setPriority(interrupt: CortexM3Interrupt, priority: Priority) void {
        ...
        const index = @intFromEnum(interrupt) - 4; // 11
        var shps: *volatile [12]u8 = @ptrCast(&SCB.SHPR1); // 编辑SCB.SHPR1
        shps[index] = @truncate(encoded); // SCB.SHPR1[11] = encoded => SHPR3.PRI_15
    }
};
...
test "aaa:" {
    ...
    std.debug.print("xxxx\n", .{});
    std.debug.print("NVIC_PRIO_BITS:{}\n", .{NVIC_PRIO_BITS});
    const pri = Priority{ .preemptive = 15, .sub = 0 };
